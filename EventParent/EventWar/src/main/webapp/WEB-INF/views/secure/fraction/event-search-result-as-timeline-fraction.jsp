...
<fmt:bundle basename="dk.schioler.shared.bits.language.LanguageResources">
		...
		<div class="container " style="text-align: center; border: solid green 2px;">
				<div class="row">
						<div class="col">&nbsp;</div>
						<c:forEach var="slot" items="${sesTimeline.slotfs}">
								<DIV class="col" style="text-align: center; border: solid blue 2px;">${slot.slotStartDateTime }</DIV>
						</c:forEach>
				</div>
				<div class="row">
						<c:forEach var="category" items="${ sesTimeline.categories}">
								<div class="col">${category};</div>
								<c:forEach var="slot" items="${sesTimeline.timelineSlots">
