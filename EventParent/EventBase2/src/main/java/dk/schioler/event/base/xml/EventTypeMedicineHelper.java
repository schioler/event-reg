...
@Component
public class EventTypeMedicineHelper implements EventTypeXMLElements {

   private Logger logger = LoggerFactory.getLogger(getClass());

   public EventTypeMedicineHelper() {

   }

   public List<BaseEvent> buildEventTypes(String srcFileName) {
      return buildEventTypes(new File(srcFileName));
   }

   public List<BaseEvent> buildEventTypes(File srcFile) {
   ...
   public List<BaseEvent> buildEventTypesFromXML(InputStream is) {
      logger.trace("buildEventTypesfromXML:" + is);
      BaseEvent rootNode = null;
      try {
...
//			logger.debug("------");
         ...
         if (ELEMENT_ROOT.equals(rootName)) {
            ...
            rootNode = new MedicineEventImpl(null, null, null, rootName, TREE_TYPE.PARENT_NAME);
   ...
   private void traverseNodes(Node xmlCurNode, BaseEvent curObject) {
      ...
      for (int idx = 0; idx < xmlChildNodes.getLength(); idx++) {
         ...
         if (Node.ELEMENT_NODE == xmlChildNode.getNodeType()) {
            ...
            if (ELEMENT_EVENT_TYPE.equalsIgnoreCase(xmlNodeName)) {
               ...
               BaseEvent eType = buildEventTypeInstance(xmlChildNode);
               logger.debug("found " + eType.toString());
               BaseEvent rootE = (BaseEvent) curObject;
               ...
               MedicineEvent dTmppl = buildEventTemplateInstance(xmlChildNode);
               logger.debug("built" + dTmppl);
               BaseEvent e = (BaseEvent) curObject;
...
//					traverseNodes(xmlChildNode, childTreeNode);
   ...
   private BaseEvent buildEventTypeInstance(Node xmlChildNode) {
      ...
      if (xmlChildNode.hasAttributes()) {
         ...
         BaseEvent et = new MedicineEventImpl(null, null, null, name, TREE_TYPE.PARENT_NAME);
//         et.setName(name);
...
//         et.setDescription(description);
   ...
   private MedicineEvent buildEventTemplateInstance(Node xmlChildNode) {
      ...
      if (xmlChildNode.hasAttributes()) {
         ...
         MedicineEvent et = new MedicineEventImpl(null, null,null,name, TREE_TYPE.PARENT_NAME);
//         et.setName(name);
//         et.setShortName(shortName);
//         et.setDescription(description);
         et.setDose(dose);
